(ns clojure-random-walk.core
  (:require [clojure-random-walk.simulation :refer [run-simulation]]
            ...
            [clojure-random-walk.image-output :refer [writeParticlesToImage create-image write-image]]))
...
(defn -main [& args]
  (do
    ...
    (let [sim-results (run-simulation)
      ...
      (write-image (writeParticlesToImage output-image 50 (sim-results :fixed-particles)) "./result.jpg"))))